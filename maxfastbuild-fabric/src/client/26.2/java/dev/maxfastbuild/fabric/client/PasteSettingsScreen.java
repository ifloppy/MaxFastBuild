...
/**
 * Paste options screen (Minecraft 26.2): filters + instant toggle shown before a paste starts.
 * Drawn with plain rects and text — no widgets — so both client versions stay identical; the
 * only version-specific part is the render/mouse entry points (extract-phase rendering here).
 */
public final class PasteSettingsScreen extends Screen {
    ...
    private void drawRow(GuiGraphicsExtractor graphics, int index, int mouseX, int mouseY) {
        ...
        graphics.text(font, LABELS[index], PasteSettingsLayout.labelX(width),
                PasteSettingsLayout.labelCenterY(index), hovered ? 0xFFFFFFFF : 0xFFE7EEF7);
    }

    private void drawButton(GuiGraphicsExtractor graphics, PasteSettingsLayout.Rect rect, String key, int textColor, int mouseX, int mouseY) {
        ...
        graphics.centeredText(font, Component.translatable(key), rect.x() + rect.w() / 2, rect.y() + 3,
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        int row = PasteSettingsLayout.hitRow(width, event.x(), event.y());
        if (row >= 0) {
            checked[row] = !checked[row];
            return true;
        }
        if (PasteSettingsLayout.confirm(width).contains(event.x(), event.y())) {
            confirm();
            return true;
        }
        if (PasteSettingsLayout.cancel(width).contains(event.x(), event.y())) {
            onClose();
            return true;
        }
