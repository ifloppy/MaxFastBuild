...
public final class SqliteTaskRepository implements TaskRepository {
    ...
    @Override public void save(BuildTask task) {
        database.transaction(connection -> {
            try (PreparedStatement s = connection.prepareStatement("""
                ...
                ON CONFLICT(id) DO UPDATE SET status=excluded.status,cursor=excluded.cursor,escrow_id=excluded.escrow_id,
                ...
                s.setString(i++, task.status().name()); s.setInt(i++, task.cursor()); s.setString(i++, task.escrowId());
    ...
    private static BuildTask read(ResultSet r) throws SQLException {
        ...
        return new BuildTask(UUID.fromString(r.getString("id")), UUID.fromString(r.getString("player_id")), r.getString("player_name"), plan,
                TaskStatus.valueOf(r.getString("status")), r.getInt("cursor"), r.getString("escrow_id"), new BigDecimal(r.getString("charged")),
